...
app.post('/api/v1/pacientes/crear', async (req, res) => {
  ...
  try {
    ...
    // Insertar campos básicos garantizados + columnas extendidas por separado
    await pool.query(
      `INSERT INTO pacientes (id, nombre, apellido, email, password_hash, activo, created_at)
       VALUES ($1,$2,$3,$4,$5,true,NOW())`,
      [id, d.nombre, d.apellido, email, hash]
    );
    // Actualizar columnas extendidas con UPDATE (tolerante a columnas faltantes)
    const updates = [
      ['tipo_documento',$1,'CC'],['numero_documento',$2,null],['fecha_nacimiento',$3,null],
      ['sexo',$4,null],['telefono',$5,null],['direccion',$6,null],['eps',$7,null],
      ['regimen',$8,null],['numero_afiliacion',$9,null],['diagnostico_hp',$10,null],
      ['grupo_hp_oms',$11,null],['clase_funcional_oms',$12,null],
      ['fecha_ingreso_programa',$13,null],['medico_tratante',$14,null],
      ['sede_ino',$15,'principal'],['antecedentes',$16,null],['observaciones_iniciales',$17,null]
    ];
    const vals = [
      d.tipo_documento||'CC', d.numero_documento||null, d.fecha_nacimiento||null,
      d.sexo||null, d.telefono||null, d.direccion||null, d.eps||null,
    ...
    const setCols = ['tipo_documento','numero_documento','fecha_nacimiento','sexo',
      'telefono','direccion','eps','regimen','numero_afiliacion','diagnostico_hp',
    ...
    try {
      await pool.query(`UPDATE pacientes SET ${setClause} WHERE id=$${setCols.length+1}`,
        [...vals, id]);
    } catch(e) {
      // Si alguna columna no existe aún, continuar sin error
