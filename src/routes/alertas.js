...
const esquemaReporte = Joi.object({
  ...
  cf_oms_paciente:     Joi.number().integer().min(1).max(4).allow(null),
}).options({ allowUnknown: true }).min(1);
...
router.get('/alertas', autenticar, requiereRol(), async (req, res) => {
  ...
  // Roles que ven TODAS las alertas del programa
  const rolesGlobales = ['administrativo','medico','cardiologo','cardiólogo','neumologo','neumólogo','enfermeria','enfermería','coordinador'];
  if (!rolesGlobales.includes((req.usuario.rol||'').toLowerCase())) {
