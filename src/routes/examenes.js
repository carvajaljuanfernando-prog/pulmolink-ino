...
router.post('/estratificacion', autenticar, requiereRol('neumólogo','cardiólogo','medico','coordinador','enfermeria','medicina_gral'), async (req, res) => {
  const { paciente_id, tipo_calculadora, resultado, clasificacion_riesgo, nt_probnp, bnp, tc6m } = req.body;
  ...
  try {
    ...
    await query(
      ...
      [req.usuario.id, paciente_id, JSON.stringify({ tipo_calculadora, clasificacion_riesgo, resultado, nt_probnp: nt_probnp||null, bnp: bnp||null, tc6m: tc6m||null })]
    );
    await query(
