...
router.get('/pacientes-lista', autenticar, requiereRol(), async (req, res) => {
  try {
    const { rows } = await query(
      `SELECT id, nombre, apellido, email, grupo_etario,
              diagnostico_hp, clase_funcional_oms
       FROM pacientes
       WHERE activo = true
...
router.post('/ecocardiograma/:pacienteId',
  autenticar, requiereRol('neumólogo','cardiólogo','medicina_gral'),
  validar(ecoSchema),
  async (req, res) => {
...
router.post('/cateterismo/:pacienteId',
  autenticar, requiereRol('neumólogo','cardiólogo'),
  validar(catSchema),
  async (req, res) => {
    const { pacienteId } = req.params;
    try {
...
// ════════════════════════════════════════════════════════════
router.get('/exportar-panel', autenticar, requiereRol('neumólogo','cardiólogo','administrativo'), async (req, res) => {
  try {
