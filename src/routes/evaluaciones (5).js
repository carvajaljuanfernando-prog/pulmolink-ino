  ...
  try {
    const { rows } = await query(
      `SELECT id, nombre, apellido, numero_documento, fecha_nacimiento,
       ...
       FROM pacientes
       WHERE activo = false OR estado = 'inactivo'
...
router.put('/:id/reactivar', autenticar, async (req, res) => {
  try {
    await query(
      `UPDATE pacientes SET activo=true, estado='activo', motivo_egreso=NULL, fecha_egreso=NULL, observaciones_egreso=NULL WHERE id=$1`,
      [req.params.id]
    );
    return res.json({ ok: true });
  } catch(err) {
