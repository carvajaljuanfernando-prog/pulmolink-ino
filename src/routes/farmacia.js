...
// POST /api/v1/farmacia/transicion
router.post('/transicion', autenticar, async (req, res) => {
  const d = req.body;
  if (!d.paciente_id) return res.status(400).json({ error: 'paciente_id requerido' });
  try {
    const id = uuidv4();
    await query(
      `INSERT INTO auditoria (usuario_tipo, usuario_id, accion, tabla, registro_id, detalle)
       VALUES ('profesional',$1,'TRANSICION_TREPROSTINIL','pacientes',$2,$3)`,
      [req.usuario.id, d.paciente_id, JSON.stringify(d)]
    );
    if (d.genera_alerta) {
      await query(
        `INSERT INTO alertas (id, paciente_id, nivel, motivo, estado, notificado_at)
         VALUES ($1,$2,'alta',$3,'pendiente',NOW())`,
        [uuidv4(), d.paciente_id,
         `Transición Treprostinil — ${d.decision === 'retorno_sc' ? 'Retorno a SC requerido' : 'Deterioro hemodinámico durante transición'}`]
      );
    }
    return res.status(201).json({ ok: true, id });
  } catch(err) {
    console.error('[POST /farmacia/transicion]', err.message);
    return res.status(500).json({ error: err.message });
  }
});

// GET /api/v1/farmacia/transicion/:pacienteId
router.get('/transicion/:pacienteId', autenticar, async (req, res) => {
  try {
    const { rows } = await query(
      `SELECT id, accion, detalle, created_at FROM auditoria
       WHERE tabla = 'pacientes'
         AND accion = 'TRANSICION_TREPROSTINIL'
         AND registro_id = $1
       ORDER BY created_at DESC LIMIT 30`,
      [req.params.pacienteId]
    );
