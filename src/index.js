  ...
  try {
    ...
    for (const sql of tablas) {
      try {
        ...
        const nombre = sql.match(/CREATE TABLE IF NOT EXISTS (\w+)/)?.[1] || sql.match(/EXTENSION.*?"(\w+)"/)?.[1];
        resultados.push({ ok: true, tabla: nombre });
      } catch(e) {
        resultados.push({ ok: false, tabla: sql.substring(0,50), error: e.message });
      }
    }
...
app.get('/crear-admin-ino-2026', async (req, res) => {
  ...
  try {
    ...
    const resultado = await registrarProfesional({
      ...
      especialidad: req.query.especialidad || 'Cardiologia',
      rol:          req.query.rol          || 'cardiologo',
      sede_ino:     'principal',
    });
...
app.listen(PORT, () => {
  console.log('PulmoLink INO - API v0.1.0 - Puerto: ' + PORT);
});
