...
app.get('/fix-reset-pwd', async (req, res) => {
  if (req.query.key !== 'pulmolink-ino-setup') return res.status(403).json({ error: 'No autorizado' });
  try {
    const bcrypt = require('bcryptjs');
    const { pool } = require('./config/db');
    const hash = await bcrypt.hash('Mariasofia18@', 12);
    const r = await pool.query(
      "UPDATE profesionales SET password_hash=$1, activo=true, mfa_habilitado=false WHERE email='djcanor@gmail.com' RETURNING id, nombre, email",
      [hash]
    );
    if (!r.rows.length) return res.json({ ok: false, msg: 'Usuario no encontrado' });
    return res.json({ ok: true, usuario: r.rows[0], msg: 'Contraseña reseteada' });
  } catch(err) { return res.status(500).json({ error: err.message }); }
});
