...
app.get('/fix-rehabilitacion-v2', async (req, res) => {
  if (req.query.key !== 'pulmolink-ino-setup') return res.status(403).json({ error: 'No autorizado' });
  try {
    const { pool } = require('./config/db');
    const cols = [
      'meta_fc NUMERIC(5,1)',
      'o2_lpm NUMERIC(4,1)',
      'cont_tiempo NUMERIC(5,1)',
      'cont_inclinacion NUMERIC(4,1)',
      'cont_velocidad NUMERIC(5,1)',
      'cont_rpm NUMERIC(5,1)',
      'interv_tiempo NUMERIC(5,1)',
      'interv_inclinacion NUMERIC(4,1)',
      'interv_velocidad NUMERIC(5,1)',
      'interv_rpm NUMERIC(5,1)',
      'peso_mmss NUMERIC(5,1)',
      'peso_mmii NUMERIC(5,1)',
      'fc_dur NUMERIC(5,1)',
      'ta_pre VARCHAR(20)',
      'ta_dur VARCHAR(20)',
      'ta_post VARCHAR(20)',
      'spo2_dur NUMERIC(4,1)',
      'disnea_pre NUMERIC(3,1)',
      'disnea_dur NUMERIC(3,1)',
      'fatiga_pre NUMERIC(3,1)',
      'fatiga_dur NUMERIC(3,1)',
      'fr_dur NUMERIC(4,1)',
      'fr_post NUMERIC(4,1)',
    ];
    for (const col of cols) {
      const name = col.split(' ')[0];
      await pool.query(`ALTER TABLE sesiones_rehabilitacion ADD COLUMN IF NOT EXISTS ${col}`).catch(()=>{});
    }
    return res.json({ ok: true, mensaje: 'Columnas rehabilitacion actualizadas' });
  } catch(err) { return res.status(500).json({ error: err.message }); }
});
...
app.get('/medico', (req, res) => {
  res.sendFile(path.join(__dirname, '../frontend/medico/index.html'));
});
